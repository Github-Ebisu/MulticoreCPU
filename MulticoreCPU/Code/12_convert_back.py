        ...
        for y in range(c):
            for z in range(no_of_cores):
                if (x * no_of_cores + z) < a:
                    OutputMatrix[x * no_of_cores + z][y] = hex(
                        inputMatrix[x * c + y][z]
                    )[2:]
    return OutputMatrix
...
data = open("../MulticoreCPU/Code/11_data_mem_out.txt", "r")
data_in = (
    data.read().strip().split("\n")[3:-1]
)  # first 3 lines contains unnecessary data
data.close()
...
writeFile = open("../MulticoreCPU/Code/13_answer_matrix_from_processor.txt", "w")
